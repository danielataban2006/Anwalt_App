...
con=sqlite3.connect("Anwalt_APP.db")   # Datenbank_Datei erstellen oder öffnen
                                      # con als Standleitung für Datenbankdatei
cursor=con.cursor()
...
create table if not exists Gegner_Partei(
...
create table if not exists Fall_wird_geöffnet_von(
    M_ID int,
    GP_ID int,
    Fall_ID int,
...
create table if not exists Fall(
    ...
    Datum_des_Lösens date default null,
    Bearbeitungsstatus varchar default 'neu' check(Bearbeitungsstatus in('neu',
...
create table if not exists Fall_benötigt_Dokument(
    D_ID int,
    F_ID int,
...
create table if not exists Aufgabe_benötigt_Dokument(
    A_ID int,
    D_ID int,
